...
async def create_canvas(session: AsyncSession, canvas: CanvasCreate) -> Canvas:
...
async def update_canvas(session: AsyncSession, canvas_update: CanvasUpdate, user_id: UUID) -> Canvas:
    canvas = await get_canvas(session, canvas_update.id, user_id)
