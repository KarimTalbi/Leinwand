...
async def node_not_found_handler(_: Request, exc: NodeNotFoundException):
    logger.debug("Node not found: %s", exc.node_id)
    return JSONResponse(status_code=404, content={"detail": str(exc)})


async def edge_not_found_handler(_: Request, exc: EdgeNotFoundException):
    logger.debug("Edge not found: %s", exc.edge_id)
    return JSONResponse(status_code=404, content={"detail": str(exc)})


async def canvas_not_found_handler(_: Request, exc: CanvasNotFoundException):
    logger.debug("Canvas not found: %s", exc.canvas_id)
    return JSONResponse(status_code=404, content={"detail": str(exc)})


async def user_already_exists_handler(_: Request, exc: UserAlreadyExistsException):
    logger.debug("User already exists: %s", exc.username)
    return JSONResponse(status_code=409, content={"detail": str(exc)})


async def inactive_user_handler(_: Request, exc: InactiveUserException):
...
def register_exception_handlers(app) -> None:
    app.add_exception_handler(NodeNotFoundException, node_not_found_handler)
    app.add_exception_handler(EdgeNotFoundException, edge_not_found_handler)
    app.add_exception_handler(CanvasNotFoundException, canvas_not_found_handler)
