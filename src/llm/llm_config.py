from enum import Enum
from typing import Any, Annotated

from pydantic import BaseModel, Field


class ModelConfig(BaseModel):

    model: str
    model_provider: str
    temperature: Annotated[float, Field(ge=0.0, le=2.0)] | None = None
    max_tokens: Annotated[int, Field(ge=0)] | None = None
    timeout: Annotated[int, Field(ge=0)] | None = None
    max_retries: Annotated[int, Field(ge=0)] | None = None
    model_kwargs: dict[str, Any] | None = None


class OpenAIModel(Enum):
    OPENAI_GPT5_MINI = ModelConfig(model="gpt-5-mini", model_provider="openai")
    OPENAI_GPT4_MINI = ModelConfig(model="gpt-4o-mini", model_provider="openai")
    OPENAI_GPT4_1_MINI = ModelConfig(model="gpt-4.1-mini", model_provider="openai")


class GoogleModel(Enum):
    GEMINI_2_FLASH = ModelConfig(
        model="gemini-2.0-flash", model_provider="google_genai"
    )
    GEMINI_2_5_FLASH = ModelConfig(
        model="gemini-2.5-flash", model_provider="google_genai"
    )
    GEMINI_2_5_FLASH_LITE = ModelConfig(
        model="gemini-2.5-flash-lite", model_provider="google_genai"
    )
    GEMINI_2_5_PRO = ModelConfig(model="gemini-2.5-pro", model_provider="google_genai")
    GEMINI_3_FLASH_PREVIEW = ModelConfig(
        model="gemini-3-flash-preview", model_provider="google_genai"
    )
    GEMINI_3_PRO_PREVIEW = ModelConfig(
        model="gemini-3-pro-preview", model_provider="google_genai"
    )
    GEMINI_3_1_FLASH_LITE_PREVIEW = ModelConfig(
        model="gemini-3.1-flash-lite-preview", model_provider="google_genai"
    )
    GEMINI_3_1_PRO_PREVIEW = ModelConfig(
        model="gemini-3.1-pro-preview", model_provider="google_genai"
    )


class AnthropicModel(Enum):
    OPUS_4_7 = ModelConfig(model="claude-opus-4-7", model_provider="anthropic")
    SONNET_4_6 = ModelConfig(model="claude-sonnet-4-6", model_provider="anthropic")
    HAIKU_4_5 = ModelConfig(model="claude-haiku-4-5", model_provider="anthropic")


class AiModel(Enum):
    OpenAI = OpenAIModel
    Google = GoogleModel
    Anthropic = AnthropicModel
